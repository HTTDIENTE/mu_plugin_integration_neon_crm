...
class WLD_TM_Log {
	...
	public static function action_shutdown() : void {
		...
		if ( static::$has_changes ) {
			foreach ( static::$logs as $handle => $contents ) {
				WLD_TM_Theme::put_file_contents( static::get_log_path( $handle ), $contents );
			}
		}
	}
	...
	protected static function get_log( $handle ) : string {
		if ( ! isset( static::$logs[ $handle ] ) ) {
			static::$logs[ $handle ] = WLD_TM_Theme::get_file_contents( static::get_log_path( $handle ) );
		}

		return static::$logs[ $handle ];
	}

	public static function get_log_path( string $handle = 'wld' ) : string {
	...
	public static function log( string $type, string $message, string $handle ) : void {
		$type     = str_pad( $type, 7 );
		$message  = wp_date( 'c' ) . ' ' . $type . ' ' . static::get_formatted_message( $message );
		$contents = WLD_TM_Theme::get_file_contents( static::get_log_path( $handle ) ) . $message . PHP_EOL . PHP_EOL;
		WLD_TM_Theme::put_file_contents( static::get_log_path( $handle ), $contents );
	}
}
